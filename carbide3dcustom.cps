...
longDescription = "Generic milling post for Carbide 3D (Grbl), with modifications from Jarvis Coghlin.";
...
properties = {
  ...
  moveForToolChange: {
    title      : "Easier tool change",
    description: "Moves to a location that is easier to do a tool change in.",
    group      : "formats",
    type       : "boolean",
    value      : true,
    scope      : "post"
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    writeln(formatComment("This is a tool change here. - Jarvis"));
    if(getProperty("moveForToolChange")){
        writeln(formatComment("I am moving for a tool change.- Jarvis "));
        writeBlock(gFormat.format(53), gFormat.format(0), xOutput.format(-40), "Y-825", zOutput.format(0));
        // G53 G0 X-40 Y-825 Z0
        // writeln(formatComment("I am moving to the front"));
        // writeBlock(gFormat.format(53), gFormat.format(53), gFormat.format(-.5));
    }
